...
ggplot(status_churn_df, aes(x = factor(Status), y = factor(Churn), fill = Count)) + 
  ...
  scale_x_discrete(labels = c("1" = "Active", "2" = "Inactive")) +  
  theme_minimal() +  
  theme(
    plot.title = element_text(hjust = 0.5, size = 16, face = "bold"), 
    axis.title.x = element_text(size = 12),
...
Status is ordinal attribute; 1 - Active ; 2 - Inactive Inactive
customers are more likely to be churned, compared to the Active
customers.\
...
ggpairs(
  ...
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, size = 3),  
