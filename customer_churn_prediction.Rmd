...
Observation:
 ...
 There is a strong positive correlation with Frequency_of_use and Seconds_of_use
 There is a positive correlation between complains and customer Churn
 

Dropping the correlated columns to avoid collinearity
Dropping Age column, as `Age` and `Age.Group` represent same feature.
```{r}
...
plot.roc(yTest , as.numeric(predictions_def_XGB$predicted) , print.auc = TRUE , col = 'blue' , add = TRUE ,  print.auc.y = 0.6)
plot.roc(yTest , as.numeric(predictions_Logistic$predicted) , print.auc = TRUE , col = 'black' , add = TRUE ,  print.auc.y = 0.5)
legend("bottom", c("XgBoost - Fine Tuned", "XGBoost", "Logistic"),
...
```
 Final Observation from Model
 Inactive Customers (Status) are most likely to be churned. 
 Customers who use for less seconds are likely to be churned.
 Customers who raised Complaints are churned compared to customers with no complaints.
